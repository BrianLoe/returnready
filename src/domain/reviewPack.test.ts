...
function populated() {
  const deductions = recordDeductions(createDemoReturnState(), [{ sourceRecordId: 'wfh-01', category: 'work-from-home', description: 'WFH hours', periodStart: '2025-07-01', periodEnd: '2026-06-30', quantity: 40, unit: 'hours', calculationMethod: 'fixed-rate', currency: 'AUD', sourceLabel: 'wfh.csv' }], 'agent', now);
...
describe('generateReviewPack for sparse drafts', () => {
  it('blocks on missing acquisition, then stores a warning-preserving evidence pack', () => {
    ...
    if (!result.ok) return;
    expect(result.value.pack.deductionEvidence[0]).toMatchObject({ sourceRecordId: 'wfh-01', quantity: 40, calculationMethod: 'fixed-rate', rateMinorPerHour: 70, claimAmountMinor: 2800, provenance: 'documentary' });
    ...
    expect(result.value.pack.unresolvedWarnings.map((issue) => issue.code).sort()).toEqual(['missing-crypto-fee']);
