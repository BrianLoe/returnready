...
describe('validateDraftReviewPack', () => {
  it('derives deduction and crypto warnings plus a missing-acquisition blocker from current facts', () => {
    const deductions = recordDeductions(createDemoReturnState(), [{ sourceRecordId: 'wfh-01', category: 'work-from-home', description: 'WFH', periodStart: '2025-07-01', periodEnd: '2026-06-30', quantity: 40, unit: 'hours', currency: 'AUD', sourceLabel: 'wfh.csv' }], 'agent', now);
    ...
    expect(result.issues.map((issue) => issue.code).sort()).toEqual(['deduction-amount-not-calculated', 'missing-acquisition', 'missing-crypto-fee']);
  });
});
