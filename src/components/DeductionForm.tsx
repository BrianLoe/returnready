  ...
  return (
    <form className="entry-form" aria-labelledby={`${formId}-heading`} onSubmit={handleSubmit}>
      ...
      <div className="form-grid">
        <label>Category<select name="category" defaultValue="work-from-home">{categories.map((value) => <option key={value} value={value} disabled={value === 'other-work-related'}>{value === 'work-from-home' ? 'Work from home' : 'Other work-related'}</option>)}</select></label>
        ...
        <label>Calculation method<select name="calculationMethod" defaultValue="fixed-rate"><option value="fixed-rate">Fixed rate</option><option value="actual-cost" disabled>Actual cost</option></select></label>
      </div>
      <p className="form-hint">Fixed rate uses 70 cents per work hour for FY2025–26. Actual cost requires itemised expense evidence.</p>
